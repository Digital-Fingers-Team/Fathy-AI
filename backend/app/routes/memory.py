...
def list_memory(
    ...
    items, total = repo.list(q=q, offset=offset, limit=limit)
    out = [
        MemoryItemOut(
            id=i.id,
            question=i.question,
...
def update_memory(item_id: int, payload: MemoryUpdateRequest, db: Session = Depends(get_db)):
    repo = MemoryRepository(db)
    item = repo.update(item_id, question=payload.question, answer=payload.answer, tags=payload.tags)
    if item is None:
        raise HTTPException(status_code=404, detail="Memory item not found")
