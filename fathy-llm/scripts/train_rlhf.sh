#!/usr/bin/env bash
set -euo pipefail

# -----------------------------
# RLHF / preference optimization launch
# -----------------------------

export PYTHONUNBUFFERED=1
export TOKENIZERS_PARALLELISM=false
export OMP_NUM_THREADS="${OMP_NUM_THREADS:-8}"
export NCCL_DEBUG="${NCCL_DEBUG:-WARN}"

export NNODES="${NNODES:-1}"
export NODE_RANK="${NODE_RANK:-0}"
export NPROC_PER_NODE="${NPROC_PER_NODE:-8}"
export MASTER_ADDR="${MASTER_ADDR:-127.0.0.1}"
export MASTER_PORT="${MASTER_PORT:-29502}"

CONFIG_PATH="${CONFIG_PATH:-fathy-llm/configs/model_medium.yaml}"
TRAIN_SCRIPT="${TRAIN_SCRIPT:-fathy_llm/train/rlhf.py}"
PREFERENCE_DATA="${PREFERENCE_DATA:-/data/rlhf/preferences.jsonl}"
SFT_CHECKPOINT="${SFT_CHECKPOINT:-/checkpoints/fathy/sft/latest}"
REWARD_MODEL="${REWARD_MODEL:-/checkpoints/fathy/reward/latest}"
OUTPUT_DIR="${OUTPUT_DIR:-/checkpoints/fathy/rlhf}"
RUN_NAME="${RUN_NAME:-fathy-rlhf}"
DIST_BACKEND="${DIST_BACKEND:-deepspeed}"

if [[ "${DIST_BACKEND}" == "deepspeed" ]]; then
  DEEPSPEED_CONFIG="${DEEPSPEED_CONFIG:-fathy-llm/configs/deepspeed_rlhf_zero3.json}"
  deepspeed \
    --num_nodes "${NNODES}" \
    --num_gpus "${NPROC_PER_NODE}" \
    --master_addr "${MASTER_ADDR}" \
    --master_port "${MASTER_PORT}" \
    "${TRAIN_SCRIPT}" \
    --config "${CONFIG_PATH}" \
    --preference_data "${PREFERENCE_DATA}" \
    --sft_checkpoint "${SFT_CHECKPOINT}" \
    --reward_model "${REWARD_MODEL}" \
    --output_dir "${OUTPUT_DIR}" \
    --run_name "${RUN_NAME}" \
    --deepspeed "${DEEPSPEED_CONFIG}"
else
  torchrun \
    --nnodes "${NNODES}" \
    --node_rank "${NODE_RANK}" \
    --nproc_per_node "${NPROC_PER_NODE}" \
    --master_addr "${MASTER_ADDR}" \
    --master_port "${MASTER_PORT}" \
    "${TRAIN_SCRIPT}" \
    --config "${CONFIG_PATH}" \
    --preference_data "${PREFERENCE_DATA}" \
    --sft_checkpoint "${SFT_CHECKPOINT}" \
    --reward_model "${REWARD_MODEL}" \
    --output_dir "${OUTPUT_DIR}" \
    --run_name "${RUN_NAME}"
fi
